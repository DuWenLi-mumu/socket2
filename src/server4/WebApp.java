...
//        servlet.put("register","server3.RegisterServlet");
    ...
    public static Servlet getServlet(String url) throws ClassNotFoundException, IllegalAccessException, InstantiationException {
        ...
        //根据字符串创建对象
        String name=null;
        if(url.matches("/\\w+.html")) {
           name=context.getServlet().get(context.getMapping().get("/*.html"));
        }
        else{
            name=name=context.getServlet().get(context.getMapping().get(url));
        }
//        System.out.println(name);
        ...
        try {
            ...
            s=(Servlet)Class.forName(name).newInstance();
            if(s==null||s.equals("")){
                throw new Exception("not found");
            }
            ...
            //e.printStackTrace();
           s=(Servlet)Class.forName("server4.LoginServlet").newInstance();
           s.setCode(404);
        }finally {
            return s;//确保空构造存在
        }
